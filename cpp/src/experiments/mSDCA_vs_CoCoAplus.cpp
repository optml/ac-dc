...
void runPCDMExperimentSparseANDTEST(L m, L n, ProblemData<int, double>& part,
		...
		ProblemData<int, double>& partTest, int sampling, int K, bool CoCoAp =
				false) {
	...
	logFile << setprecision(16) << CoCoAp << ","<<K<<","  << "0," << sampling << ","
			<< tau << "," << m << "," << n << "," << lambda << "," << primal
			<< "," << dual << "," << lonezeroloss << "," << Tprimal << ","
			<< Tdual << "," << Tlonezeroloss << endl;
	...
	std::vector < std::vector<D> > wBuffer;
	if (CoCoAp) {
		std::vector < D > wtmp(m, 0);
		for (int k = 0; k < K; k++) {
			wBuffer.push_back(wtmp);
		}
		scaling2 = K / (lambda * n)  ;
	...
	for (;;) {
		...
		if (CoCoAp) {
			...
			for (int k = 0; k < K; k++) {
				...
				cblas_set_to_zero (wBuffer[k]);

				for (int i = 0; i < tt; i++) {
					D dA = 0;

					S[i] = s * k + gsl_rng_uniform_int(gsl_rng_r, PS[k]);
					D w_xi = 0; //cblas_ddot(m, &A[S[i] * m + 0], 1, &w[0], 1);
					for (int j = part.A_csr_row_ptr[S[i]];
							j < part.A_csr_row_ptr[S[i] + 1]; j++) {
						w_xi += part.A_csr_values[j]
								* (w[part.A_csr_col_idx[j]]
										+ K*wBuffer[k][part.A_csr_col_idx[j]]);
					}

//			   h_i     =    (y_i - w_xi - x_i) / (1+   1/( lambda n Li)     );
					dA = (b[S[i]] - w_xi - x[S[i]]) / (1 + Li[S[i]] * scaling2);
					x[S[i]] = x[S[i]] + dA;
					for (int j = part.A_csr_row_ptr[S[i]];
							j < part.A_csr_row_ptr[S[i] + 1]; j++) {
						wBuffer[k][part.A_csr_col_idx[j]] += scaling * dA
								* part.A_csr_values[j];
					}
				}
			...
			for (int k=0;k<K;k++){
				...
				cblas_sum_of_vectors(w, wBuffer[k]);
			...
			for (int k = 0; k < K; k++) {
				for (int i = 0; i < tt; i++) {
					...
					do {
						...
						S[i + k * tt] = s * k
								+ gsl_rng_uniform_int(gsl_rng_r, PS[k]);
...

		{
			...
			computeDualityGapSparse(m, n, partTest, b, x, w, lambda, Tprimal,
					Tdual, Tlonezeroloss);
			cout <<CoCoAp<< "," << K<<" " << tau<< " Duality Gap: " << gap << "   " << primal << "   " << dual
					<< endl;

			logFile << setprecision(16) <<CoCoAp<<","<<K<<","<< it + tau << "," << sampling << ","
					<< tau << "," << m << "," << n << "," << lambda << ","
...
int main(int argc, char * argv[]) {
	...
	stringstream ss("");
	ss << file << "_" << K << "_mSDCAvsCoCoAplus_log";
...
//	MAXTAU = 2;
	...
	for (int tau = tauMin; tau <= MAXTAU; tau = tau * 2) {
		...
		runPCDMExperimentSparseANDTEST(m, n, part, part.b, lambda, tau, logFile,
				Li, sigma, maxTime, partTest, sampling, K, true);

		randomNumberUtil::init_random_seeds(rs);
		runPCDMExperimentSparseANDTEST(m, n, part, part.b, lambda, tau, logFile,
				Li, sigma, maxTime, partTest, sampling, K, false);
