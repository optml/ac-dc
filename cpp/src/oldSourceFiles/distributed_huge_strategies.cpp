...
//	part.lambda = 1;
	...
	settings.iterationsPerThread = 500;

//	settings.totalThreads = 2;
//	settings.iterationsPerThread = 5;
	...
	for (int exp = 0; exp < 1; exp++) {
		...
		switch (exp) {
			case 0:
				...
				settings.iterationsPerThread = 10;
				break;
			case 1:
				settings.iterationsPerThread = 1000;
				break;
			case 2:
				settings.iterationsPerThread = 10000;
				break;
			default:
				break;
		}
		...
		distributed_solver_from_multiple_sources_structured<D, L, LT>(env,
		...
		if (world.rank() == 0) {
			std::cout << "TEST " << test_case++ << " (" << settings.distributed
					...
					<< boost::timer::format(stat.time_nonstop.elapsed(), 6,
							"%w") << " " << settings.iterationsPerThread
					<< " " << settings.iters_communicate_count << "\n"
					<< std::endl;
