...
//		printf("optimal t_idx=%d, tval=%f\n", max_IDX, TVALUES[max_IDX]);
	...
	printf("Serial execution time:%f ms = %f it/sec", setialExecutionTime,
	...
	while (doworking == 1) {
		...
		for (int i = 0; i < iterations; i++) {
			...
			//			cudaMemcpy(x, E_dev, n * sizeof(float), cudaMemcpyDeviceToHost);
			//			checkCUDAError("kernel execution  : copy data Energy");
			//			for (int j = 0; j < n; j++) {
			//				printf("E[%d]=%f\n", j, x[j]);
			//			}
