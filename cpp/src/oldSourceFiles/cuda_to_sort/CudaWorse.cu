...
//#define CUDACount 5625
//#define NMAXITERKernel 5625
#define NMAXITERKernel 225
#define DIMENSIONMULTIPLIER 99
#define CUDACountTotal 9900

#define CUDACount 225
...
__global__ void RCDMKernel(float *A, int*R_Idx, int*C_Idx, int*C_Count, int* n,
	...
	__shared__ int cidx[THREDSPERBLOCK];
	__shared__ float PartDeriv[THREDSPERBLOCK];
	__shared__ float ALocal[THREDSPERBLOCK][COLUMNLENGTH];
	__shared__ int RIDX[THREDSPERBLOCK][COLUMNLENGTH];

	for (k = 0; k < CUDACountTotal; k++) {
		double d = curand_uniform_double(&localState);
		int idx = (int) (d * n[0]);
		// LOAD A, R, residuals
		cidx[threadIdx.x] = C_Idx[idx];
		PartDeriv[threadIdx.x] = 0;
		for (i = 0; i < COLUMNLENGTH; i++) {
			ALocal[threadIdx.x][i] = A[cidx[threadIdx.x] + i];
			RIDX[threadIdx.x][i] = R_Idx[cidx[threadIdx.x] + i];
		}
		for (i = 0; i < COLUMNLENGTH; i++) {
			PartDeriv[threadIdx.x] += ALocal[threadIdx.x][i]
					* residuals[RIDX[threadIdx.x][i]];
		}
		...
		tmp = LiLocal * (PartDeriv[threadIdx.x] + lambda[0]);
		if (xLocal > tmp) {
			PartDeriv[threadIdx.x] = -tmp;
		} else {
			tmp = LiLocal * (PartDeriv[threadIdx.x] - lambda[0]);
			if (xLocal < tmp) {
				PartDeriv[threadIdx.x] = -tmp;
			} else {
				PartDeriv[threadIdx.x] = -xLocal;
			}
		}
		atomicAdd(&x[idx], PartDeriv[threadIdx.x]);
		for (i = 0; i < COLUMNLENGTH; i++) {
			atomicAdd(&residuals[RIDX[threadIdx.x][i]], ALocal[threadIdx.x][i]
					* PartDeriv[threadIdx.x]);
		}
	}
//	cstate[id] = localState;
}

__global__ void ZZZZRCDMKernel(float *A, int*R_Idx, int*C_Idx, int*C_Count,
		int* n, int* m, int* nnz, float* b, float*residuals, float*x,
		float * lambda, float* Li, int* NMAX, curandState* cstate) {
	int j, i, k;
	float partialDetivative, delta, tmp;
	int id = (blockIdx.y * blockDim.x * blockDim.y + blockIdx.x * blockDim.x
			+ threadIdx.x);
	...
	for (k = 0; k < NMAXITERKernel * NMAXITER; k++) {
		double d = curand_uniform_double(&localState);
...
__global__ void XXXXMersenRandomKernelDoubleGeneratorSingle(float *A,
	...
	for (int k = 0; k < CUDACountTotal; k++) {
		temp = seed * a;
		seed = (int) (temp - mersen * floor(temp * reciprocal_m));
		//		int idx = ratioNMersen * seed;
		...
		float PartDeriv[THREDSPERBLOCK];
		//	__shared__ float ALocal[THREDSPERBLOCK][COLUMNLENGTH];
		//	__shared__ int RIDX[THREDSPERBLOCK][COLUMNLENGTH];
		PartDeriv[threadIdx.x] = 0;
		//		partialDetivative=0;
		...
		for (i = 0; i < COLUMNLENGTH; i++) {
			j = cidx + i;
			PartDeriv[threadIdx.x] += A[j] * residuals[R_Idx[j]];
		}

		//	for (i = 0; i < COLUMNLENGTH; i++) {
		//		ALocal[threadIdx.x][i] = A[cidx + i];
		//		RIDX[threadIdx.x][i] = R_Idx[cidx + i];
		//	}
		//	for (i = 0; i < COLUMNLENGTH; i++) {
		//		PartDeriv[threadIdx.x] += ALocal[threadIdx.x][i]
		//				* residuals[RIDX[threadIdx.x][i]];
		//	}
		tmp = Li[idx] * (PartDeriv[threadIdx.x] + lambda[0]);
		if (x[idx] > tmp) {
			PartDeriv[threadIdx.x] = -tmp;
		} else {
			tmp = Li[idx] * (PartDeriv[threadIdx.x] - lambda[0]);
			if (x[idx] < tmp) {
				...
				PartDeriv[threadIdx.x] = -x[idx];
			}
		}

		atomicAdd(&x[idx], PartDeriv[threadIdx.x]);
		for (i = 0; i < COLUMNLENGTH; i++) {
			atomicAdd(&residuals[R_Idx[cidx + i]], A[cidx + i]
					* PartDeriv[threadIdx.x]);
		}
	}
	//	  float LiShared[THREDSPERBLOCK];
	//	LiShared[threadIdx.x] = Li[idx];
	//	  float xLocal[THREDSPERBLOCK];
	//	xLocal[threadIdx.x] = x[idx];
	//
	//		tmp = LiShared[threadIdx.x] * (PartDeriv[threadIdx.x] + lambda[0]);
	//		if (x[idx] > tmp) {
	//			PartDeriv[threadIdx.x] = -tmp;
	//		} else {
	//			tmp = LiShared[threadIdx.x] * (PartDeriv[threadIdx.x] - lambda[0]);
	...
	//	PartDeriv[threadIdx.x] = PartDeriv[threadIdx.x] * LiShared[threadIdx.x];
	//	float deltaL = xLocal[threadIdx.x] - PartDeriv[threadIdx.x] - lambda[0]*LiShared[threadIdx.x];
	//	float deltaR = xLocal[threadIdx.x] - PartDeriv[threadIdx.x] + lambda[0]*LiShared[threadIdx.x];
	//	if (deltaL > 0)
	//		PartDeriv[threadIdx.x] = deltaL - xLocal[threadIdx.x];
	//	else if (deltaR < 0)
	//		PartDeriv[threadIdx.x] = deltaR - xLocal[threadIdx.x];
	//	else
	//		PartDeriv[threadIdx.x] = -xLocal[threadIdx.x];
	...
	//		}
	//		for (i = 0; i < COLUMNLENGTH; i++) {
	//			PartDeriv[threadIdx.x] += ALocal[i] * residuals[RIDX[i]];
	//		}
	//		for (i = 0; i < COLUMNLENGTH; i++) {
	//			PartDeriv[threadIdx.x] += ALocal[i] * residuals[RIDX[i]];
	//		}
...
__global__ void TTTTTMersenRandomKernelDoubleGeneratorSingle(float *A,
		int*R_Idx, int*C_Idx, int*C_Count, int* n, int* m, int* nnz,
		float*residuals, float*x, float * lambda, float* Li, int* NMAX) {
	unsigned int j, i;
	float partialDetivative, tmp, delta;
	...
	double const mersen = 2147483647; //ie 2**31-1
	double const reciprocal_m = 1.0 / mersen;
	int seed = (blockIdx.y * blockDim.x * blockDim.y + blockIdx.x * blockDim.x
			+ threadIdx.x);
	int threadId = seed;
	double temp;
 ...
 * 
 *      	float xLocal = x[idx];
 float lambdaOverL = lambda[0] * Li[idx];
 float alpha = partialDetivative * Li[idx];
...
void cudaMersenSolver() {
	...
	FunVal = ComputeObjectiveValue(A_h, R_Idx_h, C_Idx_h, C_Count_h, n, m, nnz,
			b, x, lambda);
	printf("INITIAL:Objective,itr,0,value,%1.16f\n", FunVal);
	float* A_d;
	float* Li_d;
	int * C_Idx_d;
	int * R_Idx_d;
	...
	FunVal = ComputeObjectiveValue(A_h, R_Idx_h, C_Idx_h, C_Count_h, n, m, nnz,
			b, x, lambda);
	printf("CPU:Objective,itr,0,value,%1.16f\n", FunVal);
	//------------------------- LOAD RESULTS FROM CUDA AND COMPARE
	float* xcuda;
	xcuda = (float*) malloc(n * sizeof(float));
	cudaMemcpy(xcuda, x_d, n * sizeof(float), cudaMemcpyDeviceToHost);
	...
	FunVal = ComputeObjectiveValue(A_h, R_Idx_h, C_Idx_h, C_Count_h, n, m, nnz,
			b, xcuda, lambda);
	printf("GPU:Objective,itr,0,value,%1.16f\n", FunVal);
	...
	printf("SERIAL NRCDM CUDA TIMMER:  %f ms, speed up:%f\n", timeSerial
			* NMAXITER / SerialExecution, timeSerial / time * NMAXITER
			/ SerialExecution);
	printf("m:%d, n:%d, p:%d\n", m, n, COLUMNLENGTH);
	printf("threds%d, dim1%d,dim2:%d\n", totalThreads, dim1, dim2);
...
void cudaMersenSolverEvolution() {
	int totalThreads = THREDSPERBLOCK;
	//	int dim1 = 1024;
	//	int dim2 = 128;
	int dim1 = 14;
	int dim2 = 100;
	int n = totalThreads * dim1 * dim2 * DIMENSIONMULTIPLIER;
	//	dim2=dim2/10;
	dim2 = dim2;
	int m = 2 * n;
	...
	double OptimalValueByNesterov = ComputeObjectiveValue(A_h, R_Idx_h,
			C_Idx_h, C_Count_h, n, m, nnz, b, xOpt, lambda);
	...
	for (i = 0; i < 1; i++) {
		...
		printf("CUDA:Objective,itr,%d,value,%1.16f,value,%1.16f,time,%f\n", i,
				FunVal, FunVal - OptimalValueByNesterov, time);
		fprintf(fp,
				"CUDA:Objective,itr,%d,value,%1.16f,value,%1.16f,time,%f\n", i,
				FunVal, FunVal - OptimalValueByNesterov, time);
		...
		printf("CPUD:Objective,itr,%d,value,%1.16f,value,%1.16f,time,%f\n", i,
				FunVal, FunVal - OptimalValueByNesterov, timeSerial);
		fprintf(fp,
				"CPUD:Objective,itr,%d,value,%1.16f,value,%1.16f,time,%f\n", i,
				FunVal, FunVal - OptimalValueByNesterov, timeSerial);
