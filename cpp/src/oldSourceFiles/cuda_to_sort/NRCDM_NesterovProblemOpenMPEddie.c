...
#include <time.h>
//ulimit -s unlimited
//gcc -fopenmp -lm  NRCDM_NesterovProblemOpenMPEddie.c -o NRCDM.out && ./NRCDM.out
//
...
#pragma omp for
				...
				t2 = clock();
				diff = ((float) t2 - (float) t1) / 1000000.0F;
				analysis[analisisIDX].elapsedTime = diff;
