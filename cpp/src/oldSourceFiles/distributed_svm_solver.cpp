...
//			"./data/a1a", argc, argv);
	...
	run_huge_exepriment<double, int, square_loss_traits>(
			"/home/d11/d11/takacn/work/data/wikipedia.svn", argc, argv);
...
// ================================LOAD DATA=======================
	...
	D optimalValue = 0;
	loadDistributedSVMData(experimentData, world.rank(), world.size(), part);

	ProblemData<L, D> part_local;
	part_local.n = part.n;
	part_local.m = 0;
	part_local.A_csc_col_ptr.resize(part.n+1,0);

	for (L col=0;col<part.n;col++){
		D nnz = part.A_csc_col_ptr[col+1]-part.A_csc_col_ptr[col];
		if (nnz > 0){

			nnz = log(part.m/nnz);


			for (L tmp = part.A_csc_col_ptr[col];tmp<part.A_csc_col_ptr[col+1];tmp++){
				part.A_csc_values[tmp]=nnz;
			}
		}
	...
	long long totalN = part.n * world.size();

	settings.totalThreads = 8;
	settings.iterationsPerThread = 1;

	settings.totalThreads = 4;
	settings.iterationsPerThread = 1;
...
//	settings.iters_bulkIterations_count = 5;
	...
	settings.iters_communicate_count = 20;
	settings.iters_bulkIterations_count = 4;
...
//			settings, stat, part_local, part, dataDistributor, rs);
//
//	if (world.rank() == 0) {
...
//			world, settings, stat, part_local, part, dataDistributor,
//			rs);
//
//	if (world.rank() == 0) {
//		std::cout << "TEST " << test_case++ << " (" << settings.distributed
...
//			env, world, settings, stat, part_local, part,
//			dataDistributor, rs);
//
//	if (world.rank() == 0) {
//		std::cout << "TEST " << test_case++ << " (" << settings.distributed
